...
class Schema
  ...
  def process_type_ref(value, type_ref)
    raise ListError::Validation, "Invalid Type Ref: Expecting '#{type_ref.to_s}' but received '#{value.class.to_s}'" if !(value.is_a?(type_ref) || (type_ref.respond_to?(:type_match?) &&  type_ref.type_match?(value))) && !value.is_a?(String)
