...
class AddItemToFieldTest < MinitestWrapper
  ...
  def test_add_field_template_field
    ...
    assert_raises(Schema::ValidationError) { add_item_to_field(@item.id, 'next_dates', 'NOT_A_DATE') }
  end

  def test_add_field_non_array_failure
    assert_raises(Schema::ValidationError) { add_item_to_field(@item.id, 'today', '100') }
  end

  def test_add_field_not_found_failure
    assert_raises(Schema::ValidationError) { add_item_to_field(@item.id, 'children', '100') }
  end
